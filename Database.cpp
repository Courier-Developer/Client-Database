...
//群里增加一个用户
int database::add_group_user(unsigned int _groupid, unsigned int _userid)
{
	add_group_user(_groupid, _userid);
}
...
//group_user_id 操作
//添加用户-群聊信息
int database::add_group_user(int _groupid, int _userid)
{
...
std::vector<MessageInfo> database::synchro_message(std::vector<MessageInfo> _meve)
{
	...
	{
		std::string sql = "select * from message_info_tb where messageid = " + tmpmes.getMessageId();
		int ret;
		ret = sqlite3_get_table(db, sql.c_str(), &result, &nrow, &ncol, &remessage);
		//debug
		printf("%s\n", remessage);
...
{
	std::string sql = "delete from message_info_tb where senderid or receiverid = " + std::to_string(_id) + "and type = " + std::to_string(_type);
	int ret;
	ret = sqlite3_exec(db, sql.c_str(), NULL, NULL, &remessage);
	//debug
	printf("%s\n", remessage);
